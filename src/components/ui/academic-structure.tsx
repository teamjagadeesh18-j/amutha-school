...
export default function AcademicStructure({
  ...
  return (
    <section id="academics" className="py-20 md:py-28 px-4 sm:px-6 md:px-8 bg-[#f7f8f9] border-t border-[#e2e8f0] font-body relative overflow-hidden">
      <div 
        className="absolute inset-0 pointer-events-none"
        style={{
          background: `radial-gradient(circle at 50% 0%, rgba(94, 106, 210, 0.05), transparent 70%)`
        }}
      />
          ...
          <div className="max-w-xl md:text-right">
            {subtitle && (
              <TextEffect 
                ...
                speedReveal={1.1} 
                speedSegment={0.3} 
                className="text-lg md:text-xl font-bold text-[#0f172a]/90 font-body mb-2"
              >
                {subtitle}
              </TextEffect>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 items-stretch">
          {cards.map((card, index) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: `0 16px 36px -10px rgba(94, 106, 210, 0.4), inset 0 1px 0 rgba(255,255,255,0.3)`
                  }}
                >
            ...
            return (
              <div
                key={index}
                className={`bg-white border border-[#e2e8f0] rounded-xl p-6 flex flex-col justify-between text-[#0f172a] transition-all duration-300 transform hover:-translate-y-1 relative overflow-hidden group ${isFirst ? 'ring-1 ring-slate-200/80 shadow-md' : 'shadow-xs hover:shadow-md'}`}
                style={{
                  boxShadow: isFirst ? `0 10px 25px -8px rgba(94, 106, 210, 0.12)` : `0 4px 20px -4px rgba(0,0,0,0.05)`
                }}
              >
                {isFirst && (
                  <div 
                ...
                <div>
                  ...
                  <TextEffect 
                    ...
                    speedReveal={1.1} 
                    speedSegment={0.3} 
                    className="text-lg font-bold text-[#0f172a] font-heading mb-2.5 leading-snug"
                  >
                    {card.title}
                  </TextEffect>
                  <TextEffect 
                ...
                <a
                  href="#connect"
                  className="inline-flex items-center justify-between text-xs font-bold transition-all duration-200 font-body mt-auto px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200/80 text-[#0f172a] hover:bg-slate-100 hover:border-slate-300 shadow-xs active:scale-[0.98] group/btn"
                >
